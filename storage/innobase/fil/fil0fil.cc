...
{
  prepare_to_close_or_leak_handle();
...
pfs_os_file_t fil_node_t::leak_handle()
{
  prepare_to_close_or_leak_handle();
...
void fil_node_t::prepare_to_close_or_leak_handle()
{
  ut_ad(mutex_own(&fil_system.mutex));
  ut_a(is_open());
  ut_a(n_pending == 0);
...
@param[in]	leak_handle	close or leak handle
@retval	leaked handle */
static
pfs_os_file_t
...
@param[in,out]	space		tablespace
@param[in]	leak_handle	leak or close handle
@retval	leaked handles */
...
@param[in,out]	leaked_handles	return leaked handles if not nullptr
@return	DB_SUCCESS or error */
